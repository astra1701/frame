<script lang="ts">
    ...
    let activeFiles = $derived(
        files.filter((f) => logs[f.id] || f.status !== FileStatus.IDLE),
    );
    ...
    let currentLogs = $derived(
        selectedLogFileId ? logs[selectedLogFileId] || [] : [],
    );
...
>
    ...
    >
        ...
        {#if activeFiles.length === 0}
            <span
                class="text-[10px] text-gray-alpha-600 uppercase tracking-widest font-medium"
            >
                No active processes
            </span>
        {/if}
                ...
                {#if currentLogs.length > 0}
                    <div class="flex flex-col">
                        {#each currentLogs as line, i}
                            <div
                                class="flex hover:bg-gray-alpha-100 rounded px-1 -mx-1 group text-[10px]"
                            >
                                <span
                                ...
                                >
                                <span
                                    class="break-all whitespace-nowrap text-gray-alpha-400"
                                    >{line}</span
                                >
                            </div>
                        {/each}
